package net.mahiron47.mathlib.types;

import net.mahiron47.mathlib.types.interfaces.ITensor;
import net.mahiron47.mathlib.types.interfaces.IVector;

public class Vec4l implements IVector {
    private long x;
    private long y;
    private long z;
    private long w;

    public Vec4l(long x, long y, long z, long w) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.w = w;
    }

    @Override
    public byte getType() {
        return ITensor.TYPE_LONG;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj) return true;
        if (!(obj instanceof IVector)) return false;

        IVector vec = (IVector) obj;
        
        return vec.getl(0) == this.x && vec.getl(1) == this.y && 
               vec.getl(2) == this.z && vec.getl(3) == this.w;
    }

    @Override
    public int hashCode() {
        int result = Long.hashCode(x);
        result = 31 * result + Long.hashCode(y);
        result = 31 * result + Long.hashCode(z);
        result = 31 * result + Long.hashCode(w);
        return result;
    }

    @Override
    public String toString() {
        return "Vec4l{" +
                "x=" + x +
                ", y=" + y +
                ", z=" + z +
                ", w=" + w +
                '}';
    }

    @Override
    public int geti(int i) {
        switch (i) {
            case 0: return (int) x;
            case 1: return (int) y;
            case 2: return (int) z;
            case 3: return (int) w;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public long getl(int i) {
        switch (i) {
            case 0: return x;
            case 1: return y;
            case 2: return z;
            case 3: return w;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public float getf(int i) {
        switch (i) {
            case 0: return (float) x;
            case 1: return (float) y;
            case 2: return (float) z;
            case 3: return (float) w;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public double getd(int i) {
        switch (i) {
            case 0: return (double) x;
            case 1: return (double) y;
            case 2: return (double) z;
            case 3: return (double) w;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public void set(int i, int a_i) {
        switch (i) {
            case 0: x = a_i; break;
            case 1: y = a_i; break;
            case 2: z = a_i; break;
            case 3: w = a_i; break;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public void set(int i, long a_i) {
        switch (i) {
            case 0: x = a_i; break;
            case 1: y = a_i; break;
            case 2: z = a_i; break;
            case 3: w = a_i; break;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public void set(int i, float a_i) {
        switch (i) {
            case 0: x = (long) a_i; break;
            case 1: y = (long) a_i; break;
            case 2: z = (long) a_i; break;
            case 3: w = (long) a_i; break;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public void set(int i, double a_i) {
        switch (i) {
            case 0: x = (long) a_i; break;
            case 1: y = (long) a_i; break;
            case 2: z = (long) a_i; break;
            case 3: w = (long) a_i; break;
            default: throw new IndexOutOfBoundsException("Index " + i + " is out of bounds for Vec4l");
        }
    }

    @Override
    public int getDimension() {
        return 4;
    }

    @Override
    public Vec4l copy() {
        return new Vec4l(x, y, z, w);
    }

    @Override
    public Mat convert() {
        long[][] matrixData = {
            {this.x},
            {this.y},
            {this.z},
            {this.w}
        };

        return new Mat(matrixData, ITensor.TYPE_LONG);
    }

    @Override
    public double getLength() {
        return Math.sqrt(x * x + y * y + z * z + w * w);
    }

    @Override
    public Vec4l getNormal() {
        double length = getLength();
        
        assert length != 0 : "Vec4l:getNormal: Cannot normalize a zero-length vector";
        
        return new Vec4l((long)(x / length), (long)(y / length), (long)(z / length), (long)(w / length));
    }

    @Override
    public Vec4l add(IVector other) {
        assert other != null : "Vec4l:add: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:add: Other vector must be of dimension 4";
        
        return new Vec4l(
            this.x + other.getl(0),
            this.y + other.getl(1),
            this.z + other.getl(2),
            this.w + other.getl(3)
        );
    }

    @Override
    public Vec4l subtract(IVector other) {
        assert other != null : "Vec4l:subtract: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:subtract: Other vector must be of dimension 4";
        
        return new Vec4l(
            this.x - other.getl(0),
            this.y - other.getl(1),
            this.z - other.getl(2),
            this.w - other.getl(3)
        );
    }

    @Override
    public Vec4l multiply(int scalar) {
        return new Vec4l(
            this.x * scalar,
            this.y * scalar,
            this.z * scalar,
            this.w * scalar
        );
    }

    @Override
    public Vec4l multiply(long scalar) {
        return new Vec4l(
            this.x * scalar,
            this.y * scalar,
            this.z * scalar,
            this.w * scalar
        );
    }

    @Override
    public Vec4l multiply(float scalar) {
        return new Vec4l(
            (long) (this.x * scalar),
            (long) (this.y * scalar),
            (long) (this.z * scalar),
            (long) (this.w * scalar)
        );
    }

    @Override
    public Vec4l multiply(double scalar) {
        return new Vec4l(
            (long) (this.x * scalar),
            (long) (this.y * scalar),
            (long) (this.z * scalar),
            (long) (this.w * scalar)
        );
    }

    @Override
    public int doti(IVector other) {
        assert other != null : "Vec4l:doti: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:doti: Other vector must be of dimension 4";

        return (int) (this.x * other.geti(0) + this.y * other.geti(1) + 
                      this.z * other.geti(2) + this.w * other.geti(3));
    }

    @Override
    public long dotl(IVector other) {
        assert other != null : "Vec4l:dotl: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:dotl: Other vector must be of dimension 4";

        return this.x * other.getl(0) + this.y * other.getl(1) + 
               this.z * other.getl(2) + this.w * other.getl(3);
    }

    @Override
    public float dotf(IVector other) {
        assert other != null : "Vec4l:dotf: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:dotf: Other vector must be of dimension 4";

        return (float) (this.x * other.getf(0) + this.y * other.getf(1) + 
                        this.z * other.getf(2) + this.w * other.getf(3));
    }

    @Override
    public double dotd(IVector other) {
        assert other != null : "Vec4l:dotd: Other vector cannot be null";
        assert other.getDimension() == 4 : "Vec4l:dotd: Other vector must be of dimension 4";

        return (double) (this.x * other.getd(0) + this.y * other.getd(1) + 
                         this.z * other.getd(2) + this.w * other.getd(3));
    }

    @Override
    public Vec4l cross(IVector other) {
        assert other != null : "Vec4l:cross: Other vector cannot be null";

        switch (other.getDimension()) {
            case 2:
                return new Vec4l(
                    -this.z * other.getl(1),
                    this.z * other.getl(0), 
                    this.x * other.getl(1) - this.y * other.getl(0),
                    this.w
                );
            case 3:
                return new Vec4l(
                    this.y * other.getl(2) - this.z * other.getl(1),
                    this.z * other.getl(0) - this.x * other.getl(2),
                    this.x * other.getl(1) - this.y * other.getl(0),
                    this.w
                );
            case 4:
                return new Vec4l(
                    this.y * other.getl(2) - this.z * other.getl(1),
                    this.z * other.getl(0) - this.x * other.getl(2),
                    this.x * other.getl(1) - this.y * other.getl(0),
                    this.w * other.getl(3)
                );
            default:
                throw new IllegalArgumentException("Vec4l:cross: Other vector must be of dimension 3 or 4");
        }
    }
}
